...
class TransformerTranslator(nn.Module):
    ...
    def embed(self, inputs):
        ...
        pos_embeddings = self.posembeddingL(pos_matrix)
...
class FullTransformerTranslator(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        src_embedded = self.srcembeddingL(src) + self.srcposembeddingL(torch.arange(src.size(1), device=self.device))
        tgt_embedded = self.tgtembeddingL(tgt) + self.tgtposembeddingL(torch.arange(tgt.size(1), device=self.device))
        
        tgt_mask = self.transformer.generate_square_subsequent_mask(tgt.size(1)).to(self.device).to(torch.bool)
        tgt_key_padding_mask = (tgt == self.pad_idx).to(torch.bool).to(self.device)
 
 
        outputs = self.transformer(src=src_embedded.permute(1, 0, 2), 
                                     tgt=tgt_embedded.permute(1, 0, 2),
                                     tgt_mask=tgt_mask, 
    ...
    def generate_translation(self, src):
        ...
        tgt = torch.full_like(src, fill_value = self.pad_idx)
        tgt[:, 0] = src[:, 0][0].item()

        # Update each output token with probabilities
        for i in range(self.max_length):       
            # Forward pass through the transformer
            outputs = self.forward(src, tgt)
            pred_prob = torch.softmax(outputs, dim=1)
            # Apply softmax to get probabilities
            outputs[:, i , :] = pred_prob[:, i, :]
            # Update target sequence only if t+1 < max_length
            if i + 1 < self.max_length:
                tgt[:, i + 1] = torch.argmax(outputs[:, i , :], dim=1)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
        return outputs
